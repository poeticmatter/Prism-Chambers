...
export function positionMatchCount(c1: ColorStr[], c2: ColorStr[]): number {
  return c1.reduce((n, c, i) => n + (c === c2[i] ? 1 : 0), 0);
}

export function createInitialState(): GameState {
  const shuffledCodes = shuffle([...CODE_COMBOS]);

  const locList = shuffledCodes.map(c => ({ code: [...c], revealedIndices: [] }));
  const locations = [
  ...
  const cards: Card[] = shuffledCodes.map((code, i) => {
